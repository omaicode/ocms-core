...
class PhpInfo
{
    ...
    public $filter = PhpInfoFilter::Modules;

    public function __construct($filter = PhpInfoFilter::Modules)
    {
        $this->filter = $filter;
    }

    public static function get($filter = PhpInfoFilter::Modules){
        return (new self($filter))->toCollection();
    }

    //for populating a dropdown
    public function filters(){
        return PhpInfoFilter::list();
    }
		...
		if(preg_match_all('#(?:<h2>(?:<a name=".*?">)?(.*?)(?:</a>)?</h2>)|(?:<tr(?: class=".*?")?><t[hd](?: class=".*?")?>(.*?)\s*</t[hd]>(?:<t[hd](?: class=".*?")?>(.*?)\s*</t[hd]>(?:<t[hd](?: class=".*?")?>(.*?)\s*</t[hd]>)?)?</tr>)#s', ob_get_clean(), $matches, PREG_SET_ORDER)){
            ...
            collect($matches)->each(function($match) use (&$phpinfo){
                if(strlen($match[1])){
                    ...
                    $phpinfo[end($keys1)][Str::slug($match[2])] = isset($match[4]) ? collect([$match[3], $match[4]]) : $match[3];
                }else{
                    $keys1 = array_keys($phpinfo);
