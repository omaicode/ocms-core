...
class Menu
{
    ...
    public function all(): Collection
    {
        ...
        if (!$prefix || $prefix === config('core.admin_prefix', 'admin')) {
        ...
        if (Auth::check() && config('cache_menu', false)) {
            $cacheKey = md5('cache-menu-' . Auth::id());
        ...
        $protocol .= config('core.admin_prefix', 'admin');
        ...
        foreach ($links as $key => &$link) {
            ...
            $link['active'] = $currentUrl == $link['url']
                || (Str::contains($link['url'], $routePrefix) && !in_array($routePrefix, ['//', '/' . config('core.admin_prefix', 'admin')]) && !Str::startsWith($link['url'], $protocol))
                || (isset($link['module-menu']) && (Str::contains($routePrefix, @$link['module-menu']) && !in_array($routePrefix, ['//', '/' . config('core.admin_prefix', 'admin')]) && !Str::startsWith($link['url'], $protocol)));
