...
class AdminTable extends TableBuilder
{
     ...
     * @var string
     */
    protected string $header_title = 'core::menu.system.administrators';
    ...
    {
        return [
            new Column("name", __('core::table.info'), fn($admin) => $this->getAdminInfo($admin)),
            new Column("email", __('core::table.email')),
            new Column("roles", __('core::table.roles'), fn($admin) => $this->getAdminRoles($admin)),
            new Column("created_at",  __('core::table.created_at'), fn($admin) => dateFormat($admin->created_at))
        ];
    }  

    private function getAdminInfo($admin)
    {
        $last_login = __('core::table.last_login_at');
