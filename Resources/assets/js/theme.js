...
w.addEventListener('load', function() {
    ...
    const selects = d.querySelectorAll('select[data-select-mode="tomSelect"]');
    const tabs    = d.querySelectorAll('.nav-link[data-bs-toggle="tab"]');
    ...
    tabs.forEach(function(tab) {
        tab.addEventListener('click', function(el) {
            ...
            params.set('tab', el.target.getAttribute('href').replace('#', ''))
            params.toString()
    ...
    const params = new URLSearchParams(location.search);
    if(params.get('tab')) {
        const tab = d.querySelector(`.nav-link[href="#${params.get('tab')}"]`)
        if(tab) {
            tab.click()
        }
    }
